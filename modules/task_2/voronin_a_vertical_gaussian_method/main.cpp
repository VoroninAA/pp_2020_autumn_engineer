...
TEST(Parallel_Operations_MPI, can_get_result_with_sequential_version) {
    int rank;
...
TEST(Parallel_Operations_MPI, can_get_result_with_parallel_version_two_unknowns) {
    ...
    if (rank == 0) {
        ...
        std::vector<double> sampleMatrix = {1,-1,-5,
                                            2,1,-7};
        ASSERT_NO_THROW(std::vector<double> result = parallelGaussianMethod(sampleMatrix,unknownNumber));
    }
}
